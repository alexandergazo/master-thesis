...
The model benefits from different attention patterns in different applications and might require that some positions attend to the whole input, for example in QA it is importand to attend to the question.
...
The main idea behind the reformer's attention is that softmax values from Equation \ref{eq:attention} are most influenced by the largest values. 
To approximate the full attention matrix focuses for each input token only on the closest tokens.
