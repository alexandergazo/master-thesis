...
Such usage of Nyst\"om approximation will not help, as the samples can only be obtained after the attention matrix is computed -- the softmax operation needs the whole matrix $QK^T$ (or at least its rows) to be complete. 
...
\end{equation}
where $\sim$ indicates a sumbsampled matrix.
...
\end{equation}
where $||\cdot||_\infty$ is the maximum absolute column sum and $Z^*$ is the result of the iterative algorithm used by the authors to compute the Moore-Penrose inverse.
Figure \ref{fig:nystrom_example} depicts anecdotal comparison between full self-attention and the approximated attention.

\begin{figure}[!htb]
        \centering
        \includegraphics{nystrom_example.pdf}
        \caption[\nystr{} Attention Example]{Comparision of the true attention and Nystr\"om approximation. Reprinted from \citep{nystrom}.}
